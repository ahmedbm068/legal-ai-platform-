...
interface Options {
    /** Staff JWT (query-param auth — browsers can't set WS headers). */
    token: string | null;
...
/** Resolve the ws(s):// URL for a case socket.
 *  In dev, Vite proxies "/api" (with ws:true) to the backend. */
function wsUrl(caseId: number, token: string): string {
    ...
    return `${base}/ws/staff/messages/${caseId}?token=${encodeURIComponent(token)}`;
}

/** Live message socket with auto-reconnect (capped exponential backoff)
 *  and a heartbeat ping. Returns connection status; consumers fall back
 *  to polling when status !== "open". */
